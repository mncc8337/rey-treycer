...
public:
    ...
    void gui(bool* lazy_ray_trace, int* frame_count, float* blur_rate, bool* force_rerender, int* frame_num, double delay, int* width, int* height, ObjectContainer* oc, Camera* camera, void (*camera_update_func)(void), Vec3* up_sky_c, Vec3* down_sky_c, int* selecting_object, std::string* selecting_object_type, bool* running) {
        //GUI part
        ...
        if(*selecting_object == -1) {
            ...
            if(*selecting_object_type == "sphere") {
                ...
                ImGui::DragFloat("radius", &radius, 0.01f, 0.00001f, 1000000);
            }
            else {
                ImGui::DragFloat3("position", position, 0.01f);
            ...
            ImGui::SliderFloat("emission_strength", &emission_strength, 0.0f, 10);
            ...
            // ensure that scale is valid
            scale[0] = fabs(scale[0]);
            scale[1] = fabs(scale[1]);
            scale[2] = fabs(scale[2]);
            if(scale[0] == 0.0f) scale[0] = 0.0001f;
            if(scale[1] == 0.0f) scale[1] = 0.0001f;
            if(scale[2] == 0.0f) scale[2] = 0.0001f;
